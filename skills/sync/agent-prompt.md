...
Then update the feature's **status**, in the `At-a-glance` table AND beside its heading, against **the boxes that feature actually has** (the tier decides which exist: a `Prototype` feature has no `Verify it` or `Test it`, an `Alpha` one has no `Test it`): `in-progress` while any box present on the feature is unticked; `done` when every box present on it is ticked. **Never reverse a `done` the engineer set**, and never require a box the feature does not carry: `done` is theirs to declare, and a skipped stage is recorded as skipped, not as incomplete.
