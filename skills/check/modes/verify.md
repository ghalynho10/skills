...
- **Before they mark it `done`, offer `/recall <feature>` once**: it checks the engineer can still explain the code that was built. Recommended when an agent wrote most of it, since nothing else in the pipeline catches that. Skippable, and never a reason to withhold `done`.
