...
- **Every user facing question carries exactly one recommended option** with a one line why (make the call, let them override). Never a neutral menu, never a cold question, never a silent decision. Two deliberate exceptions, marked as such where they appear: `/check`'s bare mode choice and `/architect`'s UI design source panel, where recommending would prejudge the whole run.
